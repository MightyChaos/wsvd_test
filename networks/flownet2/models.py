...
# def pred_flo(net, im1_list, im2_list, device_id):
#     im1_np = np.stack(im1_list, 0)
#     im2_np = np.stack(im2_list, 0)
#     ims_tns = [torch.from_numpy(im.astype(np.float32)).permute(0,3,1,2) for im in [im1_np, im2_np]]
#     ims_var = Variable(torch.stack(ims_tns, 2).contiguous().cuda(device_id), volatile=True)
#     flo_var = net.forward(ims_var)
#     flo_np = flo_var.data.cpu().numpy()
#     flo_list = np.split(flo_np, flo_np.shape[0], 0)
#     return [flo[0, ...] for flo in flo_list]

def pred_flo(net, im1_list, im2_list, device_id):
    with torch.no_grad():
        im1_np = im1_list[0]
        im2_np = im2_list[0]
        ims_tns = [torch.from_numpy(im.astype(np.float32)).permute(2,0,1) for im in [im1_np, im2_np]]
        ims_var = Variable(torch.stack(ims_tns, 1).unsqueeze(0).cuda(device_id), volatile=True)
        flo_var = net.forward(ims_var)
        flo_np = flo_var.data.cpu().numpy()
        flo_list = np.split(flo_np, flo_np.shape[0], 0)
    return [flo[0, ...] for flo in flo_list]
...
def load_flownet():
    ...
    args = Args()
    net  = FlowNet2(args)
    net.load_state_dict(torch.load('ckpts/FlowNet2_checkpoint.pth.tar')['state_dict'])
    return net
